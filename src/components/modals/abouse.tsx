...
export default function AbuseModal(): React.JSX.Element {
  ...
  const onAbuseConfirmPressed = async () => {
    ...
    try {
      ...
      if (result && result.status === ST_SUCCESS) {
        ...
        await getFetchData(
          `notify/sendPushNotification?body=${body}&title=${'Abuse'}&id=${user_id}`,
        );

        Alert.alert(
          '',
          intl.formatMessage({ id: `lang.${locale}.report_abuse_success` }),
        );
  ...
  return (
    <Modal isVisible={!!selAbuse.id} backdropColor="rgba(1, 90, 128, 0.9)">
